...
function buildScenarios(args: BuildArgs): Scenario[] {
  ...
  return [
...

    {
      ...
      summary: 'Logout usando un par de claves DPoP nuevo. aegis-core detecta el desajuste de jkt y rechaza (no se revocan los tokens).',
      outcome: 'reject',
      expectedHttp: '401 Unauthorized',
      expectedBodyShape: '{ message: "Phantom session mismatch", statusCode: 401 }',
      casesCovered: ['Caso 5'],
      ...
      renderRequest: () => ({
        ...
        headers: [
          ...
          { name: 'Content-Type', value: 'application/json' },
          { name: 'DPoP', value: 'DPoP <jwt firmado con clave FRESCA, jkt ≠ session.dpop_jkt>' },
        ],
        dpopProof: {
          header: {
            typ: 'dpop+jwt',
        ...
        jweBody: {
          ...
          plaintextShape: {
            credentials: {
              clientId: session?.channelId ?? '<channelId>',
              pass: session?.refreshToken ?? '<phantom refresh token>',
              user_check: session?.username ?? '<username>',
            },
            anti_replay: { iat: '<unix-seconds>', jti: '<UUID v4>' },
          },
        },
        aegisValidates: [
          'RateLimiterGuard (10/60s)',
          'JWE decrypt: secure_payload → JSON',
          'AntiReplayGuard: iat/jti',
          'DpopLoginGuard: typ/alg/jwk/htm/htu/iat OK',
          'UserCheckGuard: credentials.user_check',
          ...
          '  ④ dpopJkt del proof === accessSession.dpopJkt  ← FALLA AQUÍ',
          'Resultado: throw UnauthorizedException("Phantom session mismatch")',
          'Keycloak no recibe revoke; sesión Redis NO se elimina',
        ],
      }),
